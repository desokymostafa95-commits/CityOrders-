...
import { useChatThread, useSendChatMessage, useBlockChatThread, useUnblockChatThread } from '@/src/hooks/useMerchantData';
...
export default function MerchantChatScreen() {
    ...
    const blockThread = useBlockChatThread(id);
    const unblockThread = useUnblockChatThread(id);

    const thread = data?.thread;
    const isBlockedByMe = thread?.isBlockedByMerchant;
    const isBlockedByOther = thread?.isBlockedByCustomer;
    ...
    const title = useMemo(() => {
        ...
        return data.thread.customerName || data.thread.adminName || data.thread.subject;
    }, [data]);
    ...
    const handlePickImage = async () => {
        try {
            ...
            if (!result.canceled) {
                ...
                const { data: uploadRes } = await apiClient.post<{ url: string }>('/Chat/upload', formData, {
            ...
            <Stack.Screen options={{
                ...
                headerRight: () => thread ? (
                    <IconButton
                        ...
                        onPress={isBlockedByMe ? () => unblockThread.mutate() : () => blockThread.mutate()}
            ...
            >
                {(data?.messages || []).length === 0 ? (
                    <View style={styles.emptyState}>
                        ...
                        <Text variant="bodySmall" style={styles.emptyText}>اكتب رسالتك هنا وسيتم حفظها داخل النظام.</Text>
                    </View>
                ) : (
                    data?.messages.map((item) => (
                        ...
                        >
                            ...
                            >
                                ...
                                {item.body ? (
                                    <Text style={item.isMine ? styles.mineText : styles.otherText}>{item.body}</Text>
                                ) : null}
                                {item.attachmentUrl ? (
                                    <Image
                                ...
                                <Text style={item.isMine ? styles.mineTime : styles.otherTime}>
                ...
                <Surface style={styles.blockedBanner} elevation={1}>
                    {isBlockedByMe ? (
                        ...
                        <Text variant="bodyMedium" style={styles.blockedText}>
                            تم حظر هذه المحادثة من قبل العميل.
                        </Text>
                    )}
                </Surface>
            ) : (
