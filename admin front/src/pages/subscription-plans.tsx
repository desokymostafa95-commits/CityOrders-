        ...
        <div className="space-y-8">
            <div className="flex justify-between items-center">
                <div>
                    ...
                    <p className="text-slate-500 mt-1">Configure Monthly, Quarterly, and Yearly plans.</p>
                </div>
                <button
            ...
            <div className="bg-white rounded-lg shadow-sm border border-slate-200 overflow-hidden">
                <table className="w-full text-left">
                    <thead className="bg-slate-50 border-b border-slate-200">
                        <tr>
                            <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Plan Name</th>
                            <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Price (EGP)</th>
                            <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Duration</th>
                            <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Grace Days</th>
                            <th className="px-6 py-3 text-xs font-semibold text-slate-500 uppercase">Max Offers</th>
                    ...
                    <tbody className="divide-y divide-slate-200">
                        {plans?.map((plan: any) => (
                            <tr key={plan.id} className="hover:bg-slate-50 transition-colors">
                                ...
                                <td className="px-6 py-4 text-sm text-slate-600">{plan.priceEgp}</td>
                                <td className="px-6 py-4 text-sm text-slate-600">{plan.durationDays} days</td>
                                <td className="px-6 py-4 text-sm text-slate-600">{plan.graceDays} days</td>
                                <td className="px-6 py-4 text-sm text-slate-600 font-semibold">{plan.maxConcurrentOffers}</td>
            ...
            {isModalOpen && (
                <div className="fixed inset-0 bg-black/50 flex items-center justify-center p-4 z-50">
                    <div className="bg-white rounded-lg shadow-xl w-full max-w-md p-6">
                        <h2 className="text-xl font-bold mb-4">{editingPlan ? 'Edit' : 'Add'} Plan</h2>
                        <form onSubmit={handleSubmit} className="space-y-4">
                            ...
                            <div>
                                <label className="text-sm font-medium">Max Concurrent Offers (0..50)</label>
